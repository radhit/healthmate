...
import kotlinx.android.synthetic.main.activity_form_input_kala3.*
...
class FormInputKala3Activity : BaseActivity() {
    companion object {
        val EXTRA = "EXTRA"
        val EXTRA_TYPE = "EXTRA_TYPE"
        @JvmStatic
        fun getCallingIntent(activity: Activity, type: String, data: String): Intent {
            val intent = Intent(activity, FormInputKala3Activity::class.java)
            intent.putExtra(EXTRA_TYPE, type)
            intent.putExtra(EXTRA, data)
            return intent
        }
    }
    override fun getView(): Int = R.layout.activity_form_input_kala3
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        fieldTeraba.setOnClickListener {
            navigator.dataMaster(this,"teraba janin kedua",3)
        }
        ...
        call.enqueue(object : Callback<DataResponse<Any>> {
            override fun onResponse(call: Call<DataResponse<Any>>?, response: Response<DataResponse<Any>>?) {
                ...
                if (response!!.isSuccessful) {
                    ...
                    Toast.makeText(this@FormInputKala3Activity,"Terjadi kesalahan", Toast.LENGTH_LONG).show()
                }
            }

            override fun onFailure(call: Call<DataResponse<Any>>?, t: Throwable?) {
                closeLoadingDialog()
                Toast.makeText(this@FormInputKala3Activity,t!!.message, Toast.LENGTH_LONG).show()
            }
        })
    }
    ...
    private fun setDataInput() {
        ...
        incKalaModel.teraba_janin_kedua = fieldTeraba.text.toString()
        incKalaModel.pendarahan = fieldPendarahan.text.toString().toInt()
        incKalaModel.laseri = fieldLaseri.text.toString()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode== RESULT_OK){
            if (requestCode==1){
                ...
                var dataMaster = gson.fromJson(data!!.getStringExtra("data"), MasterListModel::class.java)
                fieldTeraba.setText(dataMaster.name)
            } else if (requestCode==4){
                var dataMaster = gson.fromJson(data!!.getStringExtra("data"), MasterListModel::class.java)
                fieldLaseri.setText(dataMaster.name)
            }
        }
    }
}